(() => {
  ...
  var WS_URL = "ws://127.0.0.1:18373";
  var PINTEREST_URL_MATCH = "https://www.pinterest.com/*";
  var WS_HEARTBEAT_MS = 15e3;
  var WS_ACK_STALE_MS = 45e3;
  var ws;
  var reconnectTimer;
  var heartbeatTimer;
  ...
  async function scrapeTargetUrl(rawUrl, requestId) {
    ...
    return {
      url: target.url,
      boards: [target],
      totalPins: import_lodash.default.uniqBy(pins, "pinId").length
    };
  }
  async function scrapeBoardIncrementally(tabId, board, requestId) {
    ...
    const bottomRetryWaitMs = 3e3;
    logToBridge("Starting target scrape loop", {
      slug: board.slug,
      url: board.url
    });
    for (let step = 0; step < 5e3; step += 1) {
  ...
  async function syncBoard(pins, requestId) {
    const pinList = import_lodash.default.uniqBy(
  ...
  function shouldRetrySyncResult(result) {
    ...
    return reason.includes("not ready") || reason.includes("failed to load") || reason.includes("empty") || reason.includes("wrapper") || reason.includes("save ui") || reason.includes("quick save");
  }
  function scanVisiblePinsAndScroll(board, slowRetry = false) {
    const delayMs = slowRetry ? 2e3 : 1200;
    ...
    function collectVisiblePins() {
      ...
      for (const anchor of anchors) {
        ...
        pins.push({
          ...
          title: image?.getAttribute("alt") || anchor.getAttribute("aria-label") || anchor.textContent?.replace(/\s+/g, " ").trim() || null,
  ...
  function quickSaveCurrentPin() {
    ...
    function visibleNodes(selectors) {
      return Array.from(document.querySelectorAll(selectors)).filter((node) => {
        const text3 = normalize(node.textContent || node.getAttribute("aria-label") || "");
        if (!text3) return false;
        const style = window.getComputedStyle(node);
        return style.display !== "none" && style.visibility !== "hidden";
      });
    }
    function findClickableByText(text3, selectors = 'button, [role="button"], div[tabindex="0"]') {
      const wanted = normalize(text3);
    ...
    return (async () => {
      ...
      const saveStateButton = quickSaveButtons.find((node) => {
        const text3 = normalize(node.textContent || node.getAttribute("aria-label") || "");
        return text3 === "save" || text3.includes("quick save") || text3 === "saved" || text3.includes("saved to profile") || text3.includes("saved");
      });
      if (!saveStateButton) {
  ...
  function inspectPinPageState() {
    ...
    const saveButtons = Array.from(
      ...
      const text3 = String(node.textContent || node.getAttribute("aria-label") || "").replace(/\s+/g, " ").trim().toLowerCase();
      return text3.includes("save");
    });
